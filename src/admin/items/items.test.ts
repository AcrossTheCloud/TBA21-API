...
describe('Admin Items', () => {
  ...
  test('Check that we have 6 seeds.', async () => {
    ...
    expect(result.items.objects.output.geometries.length).toEqual(6);
  });
  ...
  test('Search for Chris in creators and expect 3 results', async () => {
    ...
    expect(result.items.objects.output.geometries.length).toEqual(3);
  });
  test('Search for justice in tags and expect 5 results', async () => {
    ...
    expect(result.items.objects.output.geometries.length).toEqual(5);
  });
  ...
  test('Get item with a specific s3 key', async () => {
    const
      ...
      result = JSON.parse(response.body);
    expect(result.item.objects.output.geometries[0].properties.s3_key).toEqual('private/eu-central-1:80f1e349-677b-4aed-8b26-896570a8073c/ad742900-a6a0-11e9-b5d9-1726307e8330-rat-pet-animal-domestic-104827.jpeg');
  });
  test('Get item by its id', async () => {
    const
      ...
      response = await getItem({ queryStringParameters, path: '/admin/get' } as APIGatewayProxyEvent, {} as Context),
      result = JSON.parse(response.body);
    expect(result.item.objects.output.geometries[0].properties.id).toEqual('1');
  });
  ...
  test('Get all items with a tag of kitten', async () => {
    ...
    expect(result.items.objects.output.geometries.length).toEqual(1);
  });
  ...
  test('Contributor get item by id', async () => {
    ...
    expect(result.item.objects.output.geometries[0].properties.id).toEqual("2");
  });
});
