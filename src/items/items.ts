...
export const getItemsById = async (id: number, event: APIGatewayEvent, context: Context): Promise<APIGatewayProxyResult> => {
    if (id) {
        let
            queryString: { [name: string]: string } | null = event.queryStringParameters,
            QUERY: string = `select COUNT ( id ) OVER ( ), * from ${process.env.DB_NAME}.items items`;
        ...
        let WHERE: string = ` WHERE status=true AND id=${id}`;
